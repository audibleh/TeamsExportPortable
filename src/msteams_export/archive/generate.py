...
def generate_html_archive(exports_dir: Path, output_path: Path) -> None:
    """Read exports_dir and write a standalone HTML archive to output_path."""
    ...
    html_content = _build_html(all_chats, index_data.get("meta", {}))
...
def _clean_messages(messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for msg in messages:
        ...
        if msg.get("attachments"):
            entry["attachments"] = [
                {"label": a.get("label", ""), "href": a.get("href", ""), "kind": a.get("kind", "")}
                for a in msg["attachments"] if isinstance(a, dict)
            ]
